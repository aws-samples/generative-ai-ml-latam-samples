from aws_cdk import RemovalPolicy, Stack, aws_s3 as s3, aws_iam as iam, CfnOutput
...
class WhatsappEumConnectChatStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        lambda_functions.whatsapp_event_handler.add_environment("TABLE_NAME", tables.active_connections.table_name)
        lambda_functions.whatsapp_event_handler.add_environment("BUCKET_NAME", s3_bucket.bucket_name)
        lambda_functions.whatsapp_event_handler.add_environment("VOICE_PREFIX", "voice_")
        lambda_functions.whatsapp_event_handler.add_environment("INSTANCE_ID", INSTANCE_ID)
        lambda_functions.whatsapp_event_handler.add_environment("CONTACT_FLOW_ID", CONTACT_FLOW_ID)
        lambda_functions.whatsapp_event_handler.add_environment("CHAT_DURATION_MINUTES", str(CHAT_DURATION_MINUTES))
        lambda_functions.whatsapp_event_handler.add_environment("TOPIC_ARN", sns_topic_out.topic.topic_arn)
        ...
        lambda_functions.whatsapp_event_handler.add_to_role_policy(iam.PolicyStatement(actions=["transcribe:*"],resources=["*"]))
        ...
        lambda_functions.connect_event_handler.add_environment("TABLE_NAME", tables.active_connections.table_name)
        ...
        lambda_functions.whatsapp_event_handler.add_to_role_policy(iam.PolicyStatement(
        ...
        # Tables permissions
        tables.active_connections.grant_full_access(lambda_functions.whatsapp_event_handler)
        tables.active_connections.grant_full_access(lambda_functions.connect_event_handler)
